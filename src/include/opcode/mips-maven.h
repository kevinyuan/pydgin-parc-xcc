/*======================================================================*/
/* mips-maven.h : Mips opcode list for GDB                              */
/*======================================================================*/
/* Copyright 1993-2005, 2008
   ...
   GDB, GAS, and the GNU binutils are free software; you can
...
/* These are bit masks and shift counts to use to access the various
   ...
   Make sure you use fields that are appropriate for the instruction, of
   course.
   ...
   lower ten bits to be set too, and MIPS32 and later architectures
   allow 20 bits to be set with a signal operand (using CODE20).
...
#define OP_MASK_SEL             0x7     /* Sel field of mfcZ and mtcZ. */
...
#define OP_MASK_VECBYTE         0x7     /* Sel field is 4b, but 0x8-0xf don't select bytes. */
...
/* These are the characters which may appear in the args field of an
   instruction. They appear in the order in which the fields appear when
   the instruction is used. Commas and parentheses in the args string
   are ignored when assembling, and written into the output when
   disassembling.
   ...
   ">" shift amt between 32-63, stored after subtracting 32 (OP_*_SHAMT)
   ...
   "+t" 5 bit cop 0 destination register (OP_*_RT)
   "+T" 5 bit cop 0 destination register (OP_*_RT) - disassembly only
   ...
   "+X" Bit index field of bbit aliasing bbit32. match if
         32 <= index < 64, otherwise skips to next candidate.
   ...
   ","  separates operands
   "[]" brackets around index for vector-op scalar op specifier (vr5400)
...
/* Below this point the INSN_* values correspond to combinations of
   ISAs. They are only for use in the opcodes table to indicate
   membership of a combination of ISAs that cannot be expressed using
   the usual inclusion ordering on the above INSN_* values. */
...
/* Given INSN_ISA* values X and Y, where X ranges over INSN_ISA1 through
   INSN_ISA5_32R2 and Y ranges over INSN_ISA1 through INSN_ISA64R2, this
   table describes whether at least one of the ISAs described by X
   is/are implemented by ISA Y. (Think of Y as the ISA level supported
   by a particular core and X as the ISA level(s) at which a certain
   instruction is defined.) The ISA(s) described by X is/are implemented
   by Y iff (mips_isa_table[(Y & INSN_ISA_MASK) - 1] >> ((X &
   INSN_ISA_MASK) - 1)) & 1 is non-zero. */
...
/* CPU defines, use instead of hardcoding processor number. Keep this in
   sync with bfd/archures.c in order for machine selection to work. */
...
/* cbatten - Add maven target (following xlr/loongsoon changes) */
#define CPU_MAVEN       100

/* Test for membership in an ISA including chip specific ISAs. INSN is
   pointer to an element of the opcode table; ISA is the specified
...
/* These are the characters which may appears in the args field of a
   ...
   "U" 8 bit unsigned immediate * 0 (MIPS16OP_*_IMM8) (full 16b unsigned)
...
/* The following flags have the same value for the mips16 opcode table:
