/*======================================================================*/
/* mips-maven.c : Subroutines used for MIPS code generation             */
/*======================================================================*/
...
/* The maximum distance between the top of the stack frame and the value
   $sp has when we save and restore registers.

   The value for normal-mode code must be a SMALL_OPERAND and must
   preserve the maximum stack alignment. We therefore use a value of
   0x7ff0 in this case.
   ...
   RESTORE are not available. We can then use unextended instructions to
   save and restore registers, and to allocate and deallocate the top
   part of the frame. */
...
enum mips_builtin_type 
{
  ...
  /* The function corresponds directly to an .md pattern. There is no
     return value and the arguments are mapped to operands 0 and
     above. */
  MIPS_BUILTIN_DIRECT_NO_TARGET,

  /* The function corresponds to a comparison instruction followed by a
     mips_cond_move_tf_ps pattern. The first two arguments are the
     values to compare and the second two arguments are the vector
     operands for the movt.ps or movf.ps instruction (in assembly
     order). */
  MIPS_BUILTIN_MOVF,
  MIPS_BUILTIN_MOVT,

  /* The function corresponds to a V2SF comparison instruction. Operand
     0 of this instruction is the result of the comparison, which has
     mode CCV2 or CCV4. The function arguments are mapped to operands 1
     and above. The function's return value is an SImode boolean that is
...
{
  /* The register returned by mips16_gp_pseudo_reg; see there for
     details. */
  rtx mips16_gp_pseudo_rtx;

  /* The number of extra stack bytes taken up by register varargs. This
     area is allocated by the callee at the very top of the frame. */
  int varargs_size;
  ...
  /* The register to use as the function's global pointer, or
     INVALID_REGNUM if the function doesn't need one. */
  ...
  /* True if the function is known to have an instruction that needs
     $gp. */
  bool has_gp_insn_p;
...
struct mips_arg_info 
{
  ...
  /* The offset from the start of the stack overflow area of the
     argument's first stack word. Only meaningful when STACK_WORDS is
     nonzero. */
...
/* The largest number of operations needed to load an integer constant.
   The worst accepted case for 64-bit constants is
   LUI,ORI,SLL,ORI,SLL,ORI. When the lowest bit is clear, we can try,
   but reject a sequence with an extra SLL at the end. */
...
/* The name that appeared in the last .file directive written by
   mips_output_filename, or "" if mips_output_filename hasn't written
   anything yet. */
...
/* The nesting depth of the PRINT_OPERAND '%(', '%<' and '%['
   constructs. */
int set_noreorder;
int set_nomacro;
static int set_noat;
...
/* The architecture selected by -mipsN, or null if -mipsN wasn't
   used. */
...
/* Index [M][R] is true if register R is allowed to hold a value of mode
   M. */
...
/* A table describing all the processors GCC knows about. Names are
   ...
   To ease comparison, please keep this table in the same order as GAS's
   mips_cpu_info_table. Please also make sure that MIPS_ISA_LEVEL_SPEC
   and MIPS_ARCH_FLOAT_SPEC handle all -march options correctly. */
static const struct mips_cpu_info mips_cpu_info_table[] =
{
  ...
  /* Prefer not to use branch-likely instructions for generic MIPS32rX
     and MIPS64rX code. The instructions were officially deprecated in
     revisions 2 and earlier, but revision 3 is likely to downgrade that
     to a recommendation to avoid the instructions in code that isn't
     tuned to a specific processor. */
...
static const struct mips_rtx_cost_data mips_rtx_cost_data[PROCESSOR_MAX] =
{
  ...
  { /* XLR */
    /* Need to replace first five with the costs of calling the
       appropriate libgcc routine. */
...
/* True if -mflip-mips16 should next add an attribute for the default
   MIPS16 mode, false if it should next add an attribute for the
   opposite mode. */
...
/*----------------------------------------------------------------------*/
/* DECL is a function that needs a default "mips16" or "nomips16"
   attribute for -mflip-mips16. Return true if it should use "mips16"
   and false if it should use "nomips16". */
...
mflip_mips16_use_mips16_p( tree decl )
{
  ...
  /* Use the opposite of the command-line setting for anonymous
     decls. */
  if ( !DECL_NAME( decl ) )
...
/* mips_split_plus                                                      */
/*----------------------------------------------------------------------*/
/* If X is a PLUS of a CONST_INT, return the two terms in *BASE_PTR and
   *OFFSET_PTR. Return X in *BASE_PTR and 0 in *OFFSET_PTR otherwise. */
...
mips_build_lower( struct mips_integer_op* codes, unsigned HOST_WIDE_INT value )
{
  ...
  if ( !LUI_OPERAND( high ) && ( value & 0x18000 ) == 0x18000 ) {
    /* The constant is too complex to load with a simple LUI/ORI pair,
       ...
       possible. In this case, we know bit 16 is set and that the low 16
       bits form a negative number. If we subtract that number from
       VALUE, we will clear at least the lowest 17 bits, maybe more. */
...
/* mips_build_integer                                                   */
/*----------------------------------------------------------------------*/
/* Fill CODES with a sequence of rtl operations to load VALUE. Return
...
mips_global_symbol_p( const_rtx x )
{
  ...
  /* Weakref symbols are not TREE_PUBLIC, but their targets are global
     or weak symbols. Relocations in the object file will be against the
     target symbol, so it's that symbol's binding that matters here. */
...
/*----------------------------------------------------------------------*/
/* Return true if X should not be moved directly into register $25. We
   need this because many versions of GAS will treat "la $25,foo" as
   part of a call sequence and so allow a global "foo" to be lazily
   bound. */
...
mips_use_pic_fn_addr_reg_p( const_rtx x )
{
  ...
  if ( GET_CODE( x ) == SYMBOL_REF ) {
    /* If PLTs and copy relocations are available, the static linker
       will make sure that $25 is valid on entry to the target
       function. */
    if ( TARGET_ABICALLS_PIC0 )
      return false;

    /* Locally-defined functions use absolute accesses to set up the
       global pointer. */
...
mips_classify_symbol( const_rtx x, enum mips_symbol_context context )
{
  ...
  if ( GET_CODE( x ) == LABEL_REF ) {
    /* LABEL_REFs are used for jump tables as well as text labels. Only
       return SYMBOL_PC_RELATIVE if we know the label is in the text
       section. */
  ...
  if ( TARGET_ABICALLS_PIC2
    ...
    /* There are three cases to consider:
       ...
       R_MIPS_GOT16 relocation. We must correctly predict which of the
       two semantics (local or global) the assembler and linker will
       apply. The choice depends on the symbol's binding rather than its
       visibility.
    
       In the second case, the assembler will not use R_MIPS_GOT16
       relocations, but it chooses between local and global accesses in
       the same way as for o32 PIC.
    
       In the third case we have more freedom since both forms of access
       will work for any kind of symbol. However, there seems little
       point in doing things differently. */
  ...
  switch ( *symbol_type ) {
    ...
    case SYMBOL_64_LOW:
      /* If the target has 64-bit pointers and the object file only
         ...
         in case the 32-bit value X + OFFSET has a different sign from
         X. */
    ...
    case SYMBOL_PC_RELATIVE:
      /* Allow constant pool references to be converted to
         LABEL+CONSTANT. In this case, we no longer have access to the
         underlying constant, but the original symbol-based access was
         known to be valid. */
      if ( GET_CODE( x ) == LABEL_REF )
        return true;
    ...
    case SYMBOL_GP_RELATIVE:
      /* Make sure that the offset refers to something within the same
         object block. This should guarantee that the final PC- or
         GP-relative offset is within the 16-bit limit. */
      return offset_within_block_p( x, INTVAL( offset ) );

    case SYMBOL_GOT_PAGE_OFST:
    case SYMBOL_GOTOFF_PAGE:
      /* If the symbol is global, the GOT entry will contain the
         symbol's address, and we will apply a 16-bit offset after
         loading it. If the symbol is local, the linker should provide
         enough local GOT entries for a 16-bit offset, but larger
         offsets may lead to GOT overflow. */
      return SMALL_INT( offset );

    case SYMBOL_TPREL:
    case SYMBOL_DTPREL:
      /* There is no carry between the HI and LO REL relocations, so the
         offset is only valid if we know it won't lead to such a
         carry. */
  ...
  switch (type) {
    case SYMBOL_ABSOLUTE:
      /* When using 64-bit symbols, we need 5 preparatory instructions,
         ...
         symbols we just need a preparatory LUI for normal mode and a
         preparatory LI and SLL for MIPS16. */
    ...
    case SYMBOL_GOT_PAGE_OFST:
      /* Unless -funit-at-a-time is in effect, we can't be sure whether
         the local/global classification is accurate. The worst cases
         are:

         (1) For local symbols when generating o32 or o64 code. The
             assembler will use:
    ...
    case SYMBOL_HALF:
      /* A 16-bit constant formed by a single relocation, or a 32-bit
         ...
         constants need an "lui; addiu" sequence for normal mode and an
         "li; sll; addiu" sequence for MIPS16 mode. */
...
/* If MODE is MAX_MACHINE_MODE, return the number of instructions needed
   ...
   In both cases, treat extended MIPS16 instructions as two
   instructions. */
...
mips_cannot_force_const_mem( rtx x )
{
  ...
  /* There is no assembler syntax for expressing an address-sized high
     part. */
  if ( GET_CODE( x ) == HIGH )
    return true;

  /* As an optimization, reject constants that mips_legitimize_move can
     expand inline.

     Suppose we have a multi-instruction sequence that loads constant C
     into register R. If R does not get allocated a hard register, and R
     is used in an operand that allows both registers and memory
     references, reload will consider forcing C into memory and using
     one of the instruction's memory alternatives. Returning false here
     will force it to use an input reload instead. */
                               ...
                               bool strict_p )
{
  ...
  /* These fake registers will be eliminated to either the stack or hard
     frame pointer, both of which are usually valid base registers.
     Reload deals with the cases where the eliminated form isn't
     valid. */

  if ( regno == ARG_POINTER_REGNUM || regno == FRAME_POINTER_REGNUM )
    return true;

  /* In MIPS16 mode, the stack pointer can only address word and
     doubleword values, nothing smaller. There are two problems here:
     ...
     (b) Most uses of the stack pointer are not made explicit until
         FRAME_POINTER_REGNUM and ARG_POINTER_REGNUM have been
         eliminated. We don't know until that stage whether we'll be
         eliminating to the stack pointer (which needs the restriction)
         or the hard frame pointer (which doesn't).

     All in all, it seems more consistent to only enforce this
     restriction during and after reload. */
                     ...
                     enum machine_mode mode )
{
  ...
  /* We may need to split multiword moves, so make sure that each word
     can be accessed without inducing a carry. This is mainly needed for
     o64, which has historically only guaranteed 64-bit alignment for
     128-bit types. */
  if ( GET_MODE_SIZE(mode) > UNITS_PER_WORD
                       ...
                       enum machine_mode mode, bool strict_p )
{
  switch ( GET_CODE( x ) ) {
    ...
    case LO_SUM:
      ...
      info->reg = XEXP( x, 0 );
      info->offset = XEXP( x, 1 );
      /* We have to trust the creator of the LO_SUM to do something
         vaguely sane. Target-independent code that creates a LO_SUM
         should also create and verify the matching HIGH.
         Target-independent code that adds an offset to a LO_SUM must
         prove that the offset will not induce a carry. Failure to do
         either of these things would be a bug, and we are not required
         to check for it here. The MIPS backend itself should only
         create LO_SUMs for valid symbolic constants, with the high part
...
/* mips_lwxs_address_p                                                  */
/*----------------------------------------------------------------------*/
/* Return true if ADDR matches the pattern for the LWXS load scaled
   indexed address instruction. Note that such addresses are not
   considered legitimate in the GO_IF_LEGITIMATE_ADDRESS sense, because
   their use is so restricted. */
...
/* Return true if a value at OFFSET bytes from base register BASE can be
   ...
   Usually the offset in an unextended instruction is a 5-bit field. The
   offset is unsigned and shifted left once for LH and SH, twice for LW
   and SW, and so on. An exception is LWSP and SWSP, which have an 8-bit
   immediate field that's shifted left twice. */
...
/* mips_address_insns                                                   */
/*----------------------------------------------------------------------*/
/* Return the number of instructions needed to load or store a value of
   mode MODE at address X. Return 0 if X isn't valid for MODE. Assume
   that multiword moves may need to be split into word moves if
   MIGHT_SPLIT_P, otherwise assume that a single load or store is
   enough.
...
mips_const_insns( rtx x )
{
  ...
  switch ( GET_CODE( x ) ) {
    case HIGH:
      ...
      /* This is simply an LUI for normal mode. It is an extended LI
         followed by an extended SLL for MIPS16. */
    ...
    case CONST:
      ...
      /* Otherwise try splitting the constant into a base and offset. If
         the offset is a 16-bit value, we can load the base address into
         a register and then use (D)ADDIU to add in the offset. If the
         offset is larger, we can load the base and offset into separate
         registers and add them together with (D)ADDU. However, the
         latter is only possible before reload; during and after reload,
         we must have the option of forcing the constant into the pool
         instead. */
      split_const( x, &x, &offset );
      if ( offset != 0 ) {
...
/*----------------------------------------------------------------------*/
/* Return the number of instructions needed to implement INSN, given
   that it loads from or stores to MEM. Count extended MIPS16
   instructions as two instructions. */
...
/* Emit a move from SRC to DEST. Assume that the move expanders can
   ...
   important because, unlike emit_move_insn, the move expanders know how
   to force Pmode objects into the constant pool even when the constant
   pool address is not itself legitimate. */
...
mips_emit_binary( enum rtx_code code, rtx target, rtx op0, rtx op1 )
{
  emit_insn( gen_rtx_SET( VOIDmode, target,
                          gen_rtx_fmt_ee( code, GET_MODE( target ), 
                                          op0, op1 ) ) );
}
...
static rtx
mips_force_binary( enum machine_mode mode, enum rtx_code code, 
                   rtx op0, rtx op1 )
{
  rtx reg;
...
/* If mips_unspec_address (ADDR, SYMBOL_TYPE) is a 32-bit value, add the
   ...
   The returned expression can be used as the first operand to a
   LO_SUM. */
...
/* Return a pseudo register that contains the value of $gp throughout
   ...
   for which $gp itself is not a valid base register or addition
   operand. */
...
mips16_gp_pseudo_reg( void )
{
  ...
  /* Don't emit an instruction to initialize the pseudo register if we
     are being called from the tree optimizers' cost-calculation
     routines. */
...
mips_pic_base_register( rtx temp )
{
  ...
  if ( TARGET_USE_GOT )
    /* The first post-reload split exposes all references to $gp (both
       uses and definitions). All references must remain explicit after
       that point.
...
/* mips_split_symbol                                                    */
/*----------------------------------------------------------------------*/
/* If MODE is MAX_MACHINE_MODE, ADDR appears as a move operand,
   otherwise it appears in a MEM of that mode. Return true if ADDR is a
   legitimate constant in that context and can be split into high and
   low parts. If so, and if LOW_OUT is nonnull, emit the high part and
   store the low part in *LOW_OUT. Leave *LOW_OUT unchanged otherwise.

   TEMP is as for mips_force_temporary and is used to load the high part
   into a register.

   When MODE is MAX_MACHINE_MODE, the low part is guaranteed to be a
   legitimize SET_SRC for an .md pattern, otherwise the low part is
   guaranteed to be a legitimate address for mode MODE. */
...
/*----------------------------------------------------------------------*/
/* Return an instruction sequence that calls __tls_get_addr. SYM is the
   TLS symbol we are referencing and TYPE is the symbol type to use
   (either global dynamic or local dynamic). V0 is an RTX for the return
   value location. */
...
/*----------------------------------------------------------------------*/
/* Generate the code to access LOC, a thread-local SYMBOL_REF, and
   return its address. The return value will be both a valid address and
   a valid SET_SRC (either a REG or a LO_SUM). */
...
/* mips_force_address                                                   */
/*----------------------------------------------------------------------*/
/* If X is not a valid address for mode MODE, force it into a
   register. */
...
/* m16_check_op                                                         */
/*----------------------------------------------------------------------*/
/* We need a lot of little routines to check the range of MIPS16
   immediate operands. */
  ...
  switch ( code ) {
    ...
    case LSHIFTRT:
      /* Shifts by between 1 and 8 bits (inclusive) are unextended,
         other shifts are extended. The shift patterns truncate the
         shift count to the right size, so there are no out-of-range
         values. */
      if ( IN_RANGE( x, 1, 8 ) )
        return 0;
...
/* Return the cost of binary operation X, given that the instruction
   ...
   and that the sequence of a double-word operation has cost
   DOUBLE_COST. */
  ...
  switch ( code ) {
    case CONST_INT:
      /* Treat *clear_upper32-style ANDs as having zero cost in the
         ...
         ??? This is needed because we would otherwise try to CSE the
         constant operand. Although that's the right thing for
         instructions that continue to be a register operation
         throughout compilation, it is disastrous for instructions that
         could later be converted into a memory operation. */
      if ( TARGET_64BIT
           && outer_code == AND
    ...
    case CONST_DOUBLE:
      ...
      if ( cost > 0 ) {
        /* If the constant is likely to be stored in a GPR, SETs of
           single-insn constants are as cheap as register sets; we never
           want to CSE them.

           Don't reduce the cost of storing a floating-point zero in
           FPRs. If we have a zero in an FPR for other reasons, we can
           get better cfg-cleanup and delayed-branch results by using it
           consistently, rather than using $0 sometimes and an FPR at
           other times. Also, moves between floating-point registers are
           sometimes cheaper than (D)MTC1 $0. */
        if ( cost == 1
             && outer_code == SET
             && !( float_mode_p && TARGET_HARD_FLOAT ) )
          cost = 0;
        /* When non-MIPS16 code loads a constant N>1 times, we rarely
           want to CSE the constant itself. It is usually better to have
           N copies of the last operation in the sequence and one shared
           copy of the other operations. (Note that this is not true for
           MIPS16 code, where the final operation in the sequence is
           often an extended instruction.)

           Also, if we have a CONST_INT, we don't know whether it is for
           a word or doubleword operation, so we cannot rely on the
           result of mips_build_integer. */
    ...
    case NEG:
      ...
      if ( float_mode_p )
        ...
        *total
          = COSTS_N_INSNS( GET_MODE_SIZE(mode) > UNITS_PER_WORD ? 4 : 1 );
      return false;
    ...
    case DIV:
      ...
      if ( float_mode_p
        ...
        if ( outer_code == SQRT || GET_CODE( XEXP( x, 1 ) ) == SQRT )
          ...
          *total
            = mips_fp_div_cost(mode) + rtx_cost( XEXP( x, 1 ), 0, speed );
        return true;
      }
      /* Fall through. */
    ...
    case UMOD:
      if ( optimize_size ) {
        /* It is our responsibility to make division by a power of 2 as
           cheap as 2 register additions if we want the division
           expanders to be used for such operations; see the setting of
           sdiv_pow2_cheap in optabs.c. Using (D)DIV for MIPS16 should
           always produce shorter code than using expand_sdiv2_pow2. */
        if ( TARGET_MIPS16
             && CONST_INT_P( XEXP( x, 1 ) )
...
/* mips_subword                                                         */
/*----------------------------------------------------------------------*/
/* Return one word of double-word value OP, taking into account the
   fixed endianness of certain registers. HIGH_P is true to select the
   high part, false to select the low part. */
...
/*----------------------------------------------------------------------*/
/* Split a doubleword move from SRC to DEST. On 32-bit targets, this
   function handles 64-bit moves for which mips_split_64bit_move_p
...
mips_split_doubleword_move( rtx dest, rtx src )
{
  ...
  if ( REG_P(dest) && REGNO(dest) == MD_REG_FIRST ) {
    ...
    if ( TARGET_64BIT )
      emit_insn( gen_mthidi_ti( dest, 
                                mips_subword( src, true ), low_dest ) );

    else
      emit_insn( gen_mthisi_di( dest, 
                                mips_subword( src, true ), low_dest ) );

  }

  else if ( REG_P(src) && REGNO(src) == MD_REG_FIRST ) {
    mips_emit_move( mips_subword( dest, false ), 
  ...
  if ( dest_code == REG && GP_REG_P( REGNO(dest) ) ) {
    if ( src_code == REG ) {
      ...
      if ( REGNO(src) == LO_REGNUM ) {
        /* When generating VR4120 or VR4130 code, we use MACC and DMACC
           instead of MFLO. This avoids both the normal MIPS III HI/LO
           hazards and the errata related to -mfix-vr4130. */
        if ( ISA_HAS_MACCHI )
                          ...
                          rtx target, rtx cmp0, rtx cmp1 )
{
  ...
  /* First see if there is a MIPS instruction that can do this
     operation. If not, try doing the same for the inverse operation. If
     that also fails, force CMP1 into a register and try again. */
  mode = GET_MODE( cmp0 );
...
/* Convert a comparison into something that can be used in a branch or
   ...
   Update *CODE, *OP0 and *OP1 so that they describe the final
   comparison. If NEED_EQ_NE_P, then only EQ or NE comparisons against
   zero are possible, otherwise any standard branch condition can be
   used. The standard branch conditions are:
...
/* mips_expand_scc                                                      */
/*----------------------------------------------------------------------*/
/* Try comparing cmp_operands[0] and cmp_operands[1] using rtl code
   CODE. Store the result in TARGET and return true if successful.
...
mips_expand_conditional_trap( enum rtx_code code )
{
  ...
  /* MIPS conditional trap instructions don't have GT or LE flavors, so
     we must swap the operands and convert to LT and GE respectively. */
  switch ( code ) {
    case GT:
    case LE:
    case GTU:
                   ...
                   enum machine_mode mode, tree type, int named )
{
  ...
  switch ( mips_abi ) {
    ...
    case ABI_O64:
      /* Only leading floating-point scalars are passed in
         ...
         say, which is OK because they are not covered by the standard
         ABI. */
    ...
    case ABI_64:
      ...
      /* ??? According to the ABI documentation, the real and imaginary
         parts of complex floats should be passed in individual
         registers. The real and imaginary parts of stack arguments are
         supposed to be contiguous and there should be an extra word of
         padding at the end.
          
         This has two problems. First, it makes it impossible to use a
         single "void *" va_list type, since register and stack
         arguments are passed differently. (At the time of writing,
         MIPSpro cannot handle complex float varargs correctly.) Second,
         it's unclear what should happen when there is only one register
         free.
          
         For now, we assume that named complex floats should go into
         FPRs if there are two FPRs free, otherwise they should be
         passed in the same way as a struct containing two floats. */
      if ( info->fpr_p
           && GET_MODE_CLASS(mode) == MODE_COMPLEX_FLOAT
           && GET_MODE_UNIT_SIZE(mode) < UNITS_PER_FPVALUE ) {
  ...
  /* Set REG_OFFSET to the register count we're interested in. The EABI
     allocates the floating-point registers separately, but the other
     ABIs allocate them like integer registers. */
                   ...
                   tree type, int named )
{
  ...
  /* We will be called with a mode of VOIDmode after the last argument
     ...
     expander. If we need a MIPS16 fp_code, return a REG with the code
     stored as the mode. */
  if ( mode == VOIDmode ) {
  ...
  if ( TARGET_NEWABI
    ...
    if ( field != 0 ) {
      /* Now handle the special case by returning a PARALLEL indicating
         where each 64-bit chunk goes. INFO.REG_WORDS chunks are passed
         in registers. */
      unsigned int i;
                           ...
                           tree type, int named )
{
  ...
  /* See the comment above the CUMULATIVE_ARGS structure in mips.h for
     ...
     either the o32 or the o64 ABI, both of which pass at most 2
     arguments in FPRs. */
  if ( cum->arg_number < 2 && info.fpr_p )
    cum->fp_code += ( mode == SFmode ? 1 : 2 ) << ( cum->arg_number * 2 );

  /* Advance the register count. This has the effect of setting num_gprs
     to MAX_ARGS_IN_REGISTERS if a doubleword-aligned argument required
     us to skip the final GPR and pass the whole argument on the
     stack. */
...
/* mips_pad_reg_upward                                                  */
/*----------------------------------------------------------------------*/
/* Likewise BLOCK_REG_PADDING (MODE, TYPE, ...). Return
   !BYTES_BIG_ENDIAN if the least significant byte of the register has
   useful data. Return the opposite if the most significant byte
   does. */
...
mips_fpr_return_fields( const_tree valtype, tree *fields )
{
  ...
  i = 0;
  for ( field = TYPE_FIELDS(valtype); 
        field != 0; field = TREE_CHAIN( field ) ) 
  {
    if ( TREE_CODE( field ) != FIELD_DECL )
      continue;
...
/* mips_function_value                                                  */
/*----------------------------------------------------------------------*/
/* Implement FUNCTION_VALUE and LIBCALL_VALUE. For normal calls, VALTYPE
   is the return type and MODE is VOIDmode. For libcalls, VALTYPE is
   null and MODE is the mode of the return value. */
...
mips_function_value( const_tree valtype, enum machine_mode mode )
{
  if ( valtype ) {
    ...
    /* Since TARGET_PROMOTE_FUNCTION_RETURN unconditionally returns
...
/*----------------------------------------------------------------------*/
/* Implement TARGET_RETURN_IN_MEMORY. Under the o32 and o64 ABIs, all
   BLKmode objects are returned in memory. Under the n32, n64 and
   embedded ABIs, small structures are returned in a register. Objects
   with varying size must still be returned in memory, of course. */
...
mips_build_builtin_va_list( void )
{
  if ( EABI_FLOAT_VARARGS_P ) {
    /* We keep 3 pointers, and two offsets.
       ...
       below it. The other is advanced up the stack through the overflow
       region.
        
       The third pointer is to the bottom of the GPR save area. Since
       the FPR save area is just below it, we can address FPR slots off
       this pointer.
        
       We also keep two one-byte offsets, which are to be subtracted
       from the constant pointers to yield addresses in the GPR and FPR
       save areas. These are downcounted as float or non-float arguments
       are used, and when they get to zero, the argument must be
       obtained from the overflow region. */
...
mips_va_start( tree valist, rtx nextarg )
{
  if ( EABI_FLOAT_VARARGS_P ) {
    ...
    /* Emit code to initialize OVFL, which points to the next varargs
       stack argument. CUM->STACK_WORDS gives the number of stack words
  ...
  if ( !EABI_FLOAT_VARARGS_P )
         ...
         && GET_MODE_SIZE( TYPE_MODE(type) ) <= UNITS_PER_FPVALUE ) 
    {
      ...
      /* Overflow arguments are padded to UNITS_PER_WORD bytes (=
         PARM_BOUNDARY bits). This can be different from RSIZE in two
         cases:
         ...
         Such structures are passed in paired FPRs, so RSIZE will be 8
         bytes. However, the structure only takes up 4 bytes of memory,
         so OSIZE will only be 4.

         (2) In combinations such as -mgp64 -msingle-float
         -fshort-double. Doubles passed in registers will then take up 4
         (UNITS_PER_HWFPVALUE) bytes, but those passed on the stack take
         up UNITS_PER_WORD bytes. */
...
{
  /* If we're generating PIC, and this call is to a global function, try
     to allow its address to be resolved lazily. This isn't possible for
     sibcalls when $gp is call-saved because the value of $gp on entry
...
/* A chained list of functions for which mips16_build_call_stub has
   already generated a stub. NAME is the name of the function and
   FP_RET_P is true if the function returns a value in floating-point
   registers. */
struct mips16_stub 
{
...
static void
mips_output_32bit_xfer( char direction, unsigned int gpreg, 
                        unsigned int fpreg )
{
...
mips16_build_function_stub( void )
{
  ...
  /* Load the address of the MIPS16 function into $25. Do this first so
     that targets with coprocessor interlocks can use an MFC1 to fill
     the delay slot. */
  output_asm_insn( "la\t%^,%0", &symbol );

  /* Move the arguments from floating-point registers to general
     registers. */
  ...
  /* If the linker needs to create a dynamic symbol for the target
     function, it will associate the symbol with the stub (which, unlike
     the target function, follows the proper calling conventions). It is
     therefore useful to have a local alias for the target function, so
     that it can still be identified as MIPS16 code. As an optimization,
...
/*----------------------------------------------------------------------*/
/* The current function is a MIPS16 function that returns a value in an
   FPR. Copy the return value from its soft-float to its hard-float
   location. libgcc2 has special non-MIPS16 helper functions for each
   case. */
...
mips16_copy_fpr_return_value( void )
{
  ...
  /* The function takes arguments in $2 (and possibly $3), so calls to
     it cannot be lazily bound. */
...
/* Consider building a stub for a MIPS16 call to function *FN_PTR.
   RETVAL is the location of the return value, or null if this is a
   "call" rather than a "call_value". ARGS_SIZE is the size of the
   arguments and FP_CODE is the code built by mips_function_arg; see the
   comment above CUMULATIVE_ARGS for details.

   There are three alternatives:

   - If a stub was needed, emit the call and return the call insn
     itself.
   ...
   A stub is needed for calls to functions that, in normal mode, receive
   arguments in FPRs or return values in FPRs. The stub copies the
   arguments from their soft-float positions to their hard-float
   positions, calls the real function, then copies the return value from
   its hard-float position to its soft-float position.

   We can emit a JAL to *FN_PTR even when *FN_PTR might need a stub. If
   *FN_PTR turns out to be to a non-MIPS16 function, the linker
...
mips16_build_call_stub( rtx retval, rtx *fn_ptr, rtx args_size, int fp_code )
{
  ...
  if ( GET_CODE( fn ) != SYMBOL_REF
    ...
    /* If we are handling a floating-point return value, we need to save
       $18 in the function prologue. Putting a note on the call will
       mean that df_regs_ever_live_p ($18) will be true if the call is
       not eliminated, and we can check that in the prologue code. */
    if ( fp_ret_p )
  ...
  if ( l == NULL ) {
    ...
    stubdecl
      = build_decl( FUNCTION_DECL, stubid,
                    build_function_type( void_type_node, NULL_TREE ) );

    DECL_SECTION_NAME(stubdecl)
      = build_string( strlen(secname), secname );

    DECL_RESULT(stubdecl)
    ...
    if ( !fp_ret_p ) {
      /* Load the address of the MIPS16 function into $25. Do this first
         so that targets with coprocessor interlocks can use an MFC1 to
         fill the delay slot. */
      if ( TARGET_EXPLICIT_RELOCS ) {
    ...
    if ( !fp_ret_p ) {
      ...
      /* Save the return address in $18 and call the non-MIPS16
         function. The stub's caller knows that $18 might be clobbered,
         even though $18 is usually a call-saved register. */
  ...
  /* If we expect a floating-point return value, but we've built a stub
     which does not expect one, then we're in trouble. We can't use the
     existing stub, because it won't handle the floating-point value. We
     can't build a new stub, because the linker won't know which stub to
     use for the various calls in this object file. Fortunately, this
     case is illegal, since it means that a function was declared in two
     different ways in a single compilation. */
...
/* Expand a call of type TYPE. RESULT is where the result will go (null
   ...
   ARGS_SIZE is the size of the arguments and AUX is the value passed to
   us by mips_function_arg. LAZY_P is true if this call already involves
   a lazily-bound function address (such as when calling functions
   through a MIPS16 hard-float stub).
...
mips_split_call( rtx insn, rtx call_pattern )
{
  ...
  if ( !find_reg_note( insn, REG_NORETURN, 0 ) )
    /* Pick a temporary register that is suitable for both MIPS16 and
       ...
       values in soft-float code, so $6 is the first suitable
       candidate. */
...
mips_function_ok_for_sibcall( tree decl, tree exp ATTRIBUTE_UNUSED )
{
  ...
  /* We can't do a sibcall if the called function is a MIPS16 function
     ...
     switch the ISA mode. We only care about cases where the sibling and
     normal calls would both be direct. */
  if ( decl
       && mips_use_mips16_mode_p( decl )
       && const_call_insn_operand( XEXP( DECL_RTL( decl ), 0 ), VOIDmode ) )
    return false;

  /* When -minterlink-mips16 is in effect, assume that
     non-locally-binding functions could be MIPS16 ones unless an
     attribute explicitly tells us otherwise. */
...
/*----------------------------------------------------------------------*/
/* mips_expand_fcc_reload                                               */
/*----------------------------------------------------------------------*/
/* Emit code to move general operand SRC into condition-code register
   DEST given that SCRATCH is a scratch TFmode FPR. The sequence is:
...
/*----------------------------------------------------------------------*/
/* Emit straight-line code to move LENGTH bytes from SRC to DEST. Assume
   that the areas do not overlap. */
...
/*----------------------------------------------------------------------*/
/* Move LENGTH bytes from SRC to DEST using a loop that moves
   BYTES_PER_ITER bytes at a time. LENGTH must be at least
   BYTES_PER_ITER. Assume that the memory regions do not overlap. */
                      ...
                      HOST_WIDE_INT bytes_per_iter )
{
  ...
  /* Calculate the value that SRC_REG should have after the last
     iteration of the loop. */
  final_src
    = expand_simple_binop( Pmode, PLUS, src_reg, GEN_INT(length),
                           0, 0, OPTAB_WIDEN );
...
/* Expand a QI or HI mode atomic memory operation.
   ...
   GENERATOR contains a pointer to the gen_* function that generates the
   SI mode underlying atomic operation using masks that we calculate.

   RESULT is the return register for the operation. Its value is NULL if
   unused.
   ...
   NEWVAL is the optional second operand for the operation. Its value is
   NULL if unused. */
...
/* Return true if it is possible to use left/right accesses for a
   bitfield of WIDTH bits starting BITPOS bits into *OP. When returning
   true, update *OP, *LEFT and *RIGHT as follows:

   *OP is a BLKmode reference to the whole field.

   *LEFT is a QImode reference to the first byte if big endian or the
   last byte if little endian. This address can be used in the left-side
                        ...
                        rtx *left, rtx *right )
{
  ...
  /* Adjust *OP to refer to the whole field. This also has the effect of
     legitimizing *OP's address for BLKmode, possibly simplifying it. */
  *op = adjust_address( *op, BLKmode, 0 );
  set_mem_size( *op, GEN_INT( width / BITS_PER_UNIT ) );

  /* Get references to both ends of the field. We deliberately don't use
     the original QImode *OP for FIRST since the new BLKmode one might
     have a simpler address. */
                                   ...
                                   HOST_WIDE_INT bitpos )
{
  ...
  /* After the above adjustment, the destination must be the same width
     as the source. */
...
bool
mask_low_and_shift_p( enum machine_mode mode, rtx mask, 
                      rtx shift, int maxlen )
{
...
static void
mips_print_operand_reloc( FILE* file, rtx op, 
                          enum mips_symbol_context context,
                          const char** relocs )
{
  enum mips_symbol_type symbol_type;
...
static void
mips_print_int_branch_condition( FILE *file, enum rtx_code code,
                                 int letter )
{
  switch ( code ) {
    case EQ:
...
static void
mips_print_float_branch_condition( FILE *file, enum rtx_code code, 
                                   int letter )
{
  switch ( code ) {
    case EQ:
...
/* mips_print_operand                                                   */
/*----------------------------------------------------------------------*/
/* Implement the PRINT_OPERAND macro. The MIPS-specific operand codes
   are:
   ...
   'h'  Print the high-part relocation associated with OP, after
          stripping any outermost HIGH.
   'R'  Print the low-part relocation associated with OP.
   'C'  Print the integer branch condition for comparison OP.
   'N'  Print the inverse of the int branch condition for comparison OP.
...
mips_print_operand( FILE *file, rtx op, int letter )
{
  ...
  switch ( letter ) {
    ...
    case 'h':
      if ( code == HIGH )
        op = XEXP( op, 0 );
      mips_print_operand_reloc( file, op, SYMBOL_CONTEXT_LEA,
                                mips_hi_relocs );
      break;
    ...
    case 'N':
      mips_print_int_branch_condition( file, reverse_condition( code ), 
                                       letter );
      break;
...
/* Implement TARGET_ASM_FUNCTION_RODATA_SECTION.
   ...
   read-only part of a DSO. Handle such cases by selecting a normal data
   section instead of a read-only one. The logic apes that in
...
/*----------------------------------------------------------------------*/
/* The MIPS debug format wants all automatic variables and arguments to
   be in terms of the virtual frame pointer (stack pointer before any
   adjustment in the function), while the MIPS 3.0 linker wants the
   frame pointer to be the stack pointer after the initial adjustment.
   So, we do the adjustment here. The arg pointer (which is eliminated)
   points to the virtual frame pointer, while the frame pointer (which
   may be eliminated) points to the stack pointer after the initial
   adjustments. */
...
void
mips_declare_object( FILE *stream, const char* name, 
...
/*----------------------------------------------------------------------*/
/* Declare a common object of SIZE bytes using asm directive
   INIT_STRING. NAME is the name of the object and ALIGN is the required
   alignment in bytes. TAKES_ALIGNMENT_P is true if the directive takes
   a third alignment argument. */
...
/* Implement ASM_OUTPUT_ALIGNED_DECL_COMMON. This is usually the same as
   the elfos.h version, but we also need to handle
...
/*----------------------------------------------------------------------*/
/* Implement ASM_DECLARE_OBJECT_NAME. This is like most of the standard
   ELF definitions except that it uses mips_declare_object to emit the
   label. */
...
mips_file_start( void )
{
  ...
  if ( !TARGET_IRIX ) {
    /* Record the ABI itself. Modern versions of binutils encode this
       information in the ELF header flags, but GDB needs the
    ...
    /* There is no ELF header flag to distinguish long32 forms of the
       EABI from long64 forms. Emit a special section to help tools such
       as GDB. Do the same for o64, which is sometimes used with
       -mlong64. */
...
/* A list of the registers that can be saved by the MIPS16e SAVE
   instruction, ordered from the uppermost in memory to the lowest in
   memory. */
...
/*----------------------------------------------------------------------*/
/* Return a simplified form of X using the register values in
   REG_VALUES. REG_VALUES[R] is the last value assigned to hard register
   R, or null if R has not been modified.

   This function is rather limited, but is good enough for our
   purposes. */
...
/*----------------------------------------------------------------------*/
/* Return RTL for a MIPS16e SAVE or RESTORE instruction; RESTORE_P says
   which. The instruction must:
   ...
   handle. Destructively modify *MASK_PTR so that it contains the
   registers that still need to be saved or restored. The caller can
   save these registers in the memory immediately below *OFFSET_PTR,
   which is a byte offset from the bottom of the allocated stack
   area. */
                            ...
                            HOST_WIDE_INT size )
{
  ...
  /* Tell the caller what offset it should use for the remaining
     registers. */
...
/*----------------------------------------------------------------------*/
/* Return the assembly instruction for a MIPS16e SAVE or RESTORE
   instruction. PATTERN and ADJUST are as for
   mips16e_save_restore_pattern_p. */
...
mips16e_output_save_restore( rtx pattern, HOST_WIDE_INT adjust )
{
  ...
  /* Save or restore registers in the range $s2...$s8, which
     mips16e_s2_s8_regs lists in decreasing order. Note that this is a
     software register range; the hardware registers are not numbered
     consecutively. */
...
/*----------------------------------------------------------------------*/
/* Return true if the current function returns its value in a
   floating-point register in MIPS16 mode. */
...
/* mips_global_pointer                                                  */
/*----------------------------------------------------------------------*/
/* Return the register that should be used as the global pointer within
   this function. Return INVALID_REGNUM if the function doesn't need a
   global pointer. */
...
mips_global_pointer( void )
{
  ...
  /* There's no need to initialize $gp if it isn't referenced now, and
     if we can be sure that no new references will be added during or
     after reload. */
  if ( !df_regs_ever_live_p( GLOBAL_POINTER_REGNUM )
    ...
    /* We need to handle the following implicit gp references:
       ...
       - Reload can sometimes introduce constant pool references into a
         function that otherwise didn't need them. For example, suppose
         we have an instruction like:
       ...
       - MIPS16 functions that return in FPRs need to call an external
         libgcc routine. */
...
{
  /* call_insns preserve $28 unless they explicitly say otherwise, so
     call_really_used_regs[] treats $28 as call-saved. However, we want
     the ABI property rather than the default call_insn property
     here. */
...
mips_save_reg_p( unsigned int regno )
{
  if ( mips_cfun_call_saved_reg_p( regno ) ) {
    ...
    /* Save both registers in an FPR pair if either one is used. This is
       needed for the case when MIN_FPRS_PER_FMT == 1, which allows the
       odd register to be used without the even register. */
    if ( FP_REG_P( regno )
         && MAX_FPRS_PER_FMT == 2
...
/* Populate the current function's mips_frame_info structure.
   ...
   Dynamic stack allocations such as alloca insert data at point P. They
   decrease stack_pointer_rtx but leave frame_pointer_rtx and
...
mips_compute_frame_info( void )
{
  ...
  /* The first STARTING_FRAME_OFFSET bytes contain the outgoing argument
     area and the $gp save slot. This area isn't needed in leaf
     functions, but if the target-independent frame size is nonzero,
     we're committed to allocating it anyway. */
  ...
  /* The MIPS16e SAVE and RESTORE instructions have two ranges of
     registers: $a3-$a0 and $s2-$s8. If we save one register in the
     range, we must save all later registers too. */
  if ( GENERATE_MIPS16E_SAVE_RESTORE ) {
  ...
  /* Move above the callee-allocated area for pretend stack
     arguments. */
  offset += crtl->args.pretend_args_size;
  frame->total_size = offset;

  /* Work out the offsets of the save areas from the top of the
     frame. */
...
{
  /* If the function contains dynamic stack allocations, we need to use
     the frame pointer to access the static parts of the frame. */
  if ( cfun->calls_alloca )
    return true;

  /* In MIPS16 mode, we need a frame pointer for a large frame;
     otherwise, reload may be unable to compute the address of a local
     variable, since there is no way to add a large constant to the
     stack pointer without using a second temporary register. */
  if ( TARGET_MIPS16 ) {
    mips_compute_frame_info();
...
/* Implement INITIAL_ELIMINATION_OFFSET. FROM is either the frame
   pointer or argument pointer. TO is either the stack pointer or hard
   frame pointer. */
...
mips_set_return_address( rtx address, rtx scratch )
{
  ...
  mips_emit_move( gen_frame_mem( GET_MODE( address ), slot_address ),
                  address );
}
...
/* mips_cprestore_slot                                                  */
/*----------------------------------------------------------------------*/
/* Return a MEM rtx for the cprestore slot, using TEMP as a temporary
   base register if need be. */
...
/* mips_restore_gp                                                      */
/*----------------------------------------------------------------------*/
/* Restore $gp from its save slot, using TEMP as a temporary base
   register if need be. This function is for o32 and o64 abicalls
   only. */
...
/*----------------------------------------------------------------------*/
/* Use FN to save or restore register REGNO. MODE is the register's mode
   and OFFSET is the offset of its save slot from the current stack
   pointer. */
...
mips_for_each_saved_reg( HOST_WIDE_INT sp_offset, mips_save_restore_fn fn )
{
  ...
  /* Save registers starting from high to low. The debuggers prefer at
     least the return register be stored at func+4, and also it allows
     us not to need a nop in the epilogue if at least one register is
     reloaded in addition to return address. */
...
/* If we're generating n32 or n64 abicalls, and the current function
   does not use $28 as its global pointer, emit a cplocal directive. Use
   pic_offset_table_rtx as the argument to the directive. */
                               ...
                               HOST_WIDE_INT size ATTRIBUTE_UNUSED )
{
  ...
  /* Get the function name the same way that toplev.c does before
     calling assemble_start_function. This is needed so that the name
     used here exactly matches the name used in
  ...
  if ( mips_current_loadgp_style() == LOADGP_OLDABI ) {
    ...
    /* .cpload must be in a .set noreorder but not a .set nomacro
       block. */
...
mips_save_reg( rtx reg, rtx mem )
{
  if ( GET_MODE( reg ) == DFmode && !TARGET_FLOAT64 ) {
    ...
    if ( TARGET_MIPS16
         ...
         && !M16_REG_P( REGNO( reg ) ) ) {
      /* Save a non-MIPS16 register by moving it through a temporary. We
         don't need to do this for $31 since there's a special
         instruction for it. */
...
mips_emit_loadgp( void )
{
  ...
  switch ( mips_current_loadgp_style() ) {
    ...
    case LOADGP_NEWABI:
      ...
      emit_insn( Pmode == SImode
                 ? gen_loadgp_newabi_si( pic_reg, offset, 
                                         incoming_address )
                 : gen_loadgp_newabi_di( pic_reg, offset, 
                                         incoming_address ) );
      break;
  ...
  /* Emit a blockage if there are implicit uses of the GP register. This
     includes profiled functions, because FUNCTION_PROFILE uses a jal
     macro. */
...
mips_expand_prologue( void )
{
  ...
  /* Save the registers. Allocate up to MIPS_MAX_FIRST_STACK_STEP bytes
     beforehand; this is enough to cover the register save area without
     going out of range. */
  ...
  if ( size > 0 ) {
    if ( SMALL_OPERAND( -size ) )
      ...
      if ( TARGET_MIPS16 ) {
        /* There are no instructions to add or subtract registers from
           the stack pointer, so use the frame pointer as a temporary.
           We should always be using a frame pointer in this case
           anyway. */
...
{
  /* There's no MIPS16 instruction to load $31 directly. Load into $7
     instead and adjust the return insn appropriately. */
       ...
       && ( INTEGRAL_MODE_P(mode) || ALL_FIXED_POINT_MODE_P(mode) ) ) 
  {
    ...
    if ( MD_REG_P( regno ) ) {
      ...
      if ( size <= UNITS_PER_WORD * 2 )
        return regno 
                 == ( size <= UNITS_PER_WORD ? LO_REGNUM : MD_REG_FIRST );
    }
    else {
...
/*----------------------------------------------------------------------*/
/* Return true if moves in mode MODE can use the FPU's mov.fmt
   instruction. */
...
mips_preferred_reload_class( rtx x, enum reg_class rclass )
{
  if ( mips_dangerous_for_la25_p(x)
       && reg_class_subset_p( LEA_REGS, rclass ) )
    return LEA_REGS;
                         ...
                         enum reg_class from, enum reg_class to )
{
  ...
  /* Handle moves that can be done without using general-purpose
     registers. */
  if ( from == FP_REGS ) {
...
mips_ira_cover_classes( void )
{
  ...
  /* Don't allow the register allocators to use LO and HI in MIPS16
     mode, which has no MTLO or MTHI instructions. Also, using
     GR_AND_ACC_REGS as a cover class only works well when we keep
...
/* Return the register class required for a secondary register when
   copying between one of the registers in RCLASS and value X, which has
...
static int
mips_mode_rep_extended( enum machine_mode mode, 
...
mips_adjust_insn_length( rtx insn, int length )
{
  ...
  /* In order to make it easier to share MIPS16 and non-MIPS16 patterns,
     the .md file length attributes are 4-based for both modes. Adjust
     the MIPS16 ones here. */
  if ( TARGET_MIPS16 )
    length /= 2;
...
/* Return the assembly code for INSN, which has the operands given by
   OPERANDS, and which branches to OPERANDS[1] if some condition is
   true. BRANCH_IF_TRUE is the asm template that should be used if
   OPERANDS[1] is in range of a direct branch. BRANCH_IF_FALSE is an
   inverted version of BRANCH_IF_TRUE. */
                                ...
                                const char* branch_if_false )
{
  ...
  if ( length <= 8 ) {
    ...
    mips_branch_likely = ( final_sequence 
...
/*----------------------------------------------------------------------*/
/* Return the assembly code for INSN, which branches to OPERANDS[1] if
   some ordering condition is true. The condition is given by
   OPERANDS[0] if !INVERTED_P, otherwise it is the inverse of
   OPERANDS[0]. OPERANDS[2] is the comparison's first operand; its
   second is always zero. */

const char*
mips_output_order_conditional_branch( rtx insn, rtx *operands, 
                                      bool inverted_p )
{
  const char* branch[2];
...
/*----------------------------------------------------------------------*/
/* Return the assembly code for __sync_*() loop LOOP. The loop should
   support both normal and likely branches, using %? and %~ where
   appropriate. */
...
/*----------------------------------------------------------------------*/
/* Return the assembly code for DIV or DDIV instruction DIVISION, which
   has the operands given by OPERANDS. Add in a divide-by-zero check if
   needed.
   ...
   division. When a divide-by-zero check is needed, this nop can be used
   to fill the branch delay slot.

   [1] If a double-word or a variable shift executes immediately after
       starting an integer division, the shift may give an incorrect
       result. See quotations of errata #16 and #28 from "MIPS
   ...
   [2] A similar bug to [1] exists for all revisions of the
       ...
       "19. In this following sequence:
            ...
            pipeline, then the following double shift would end up with
            an incorrect result.
   ...
   [3] From "MIPS R4000PC/SC Errata, Processor Revision 2.2 and 3.0"
       ...
       "52. R4000SC: This bug does not apply for the R4000PC.
            ...
            1) If the instruction just after divide takes an RF
               exception (tlb-refill, tlb-invalid) and gets an
               instruction cache miss (both primary and secondary) and
               the line which is currently in secondary cache at this
               index had the first data word, where the bits 5..2 are
            ...
            2) If the divide is in the taken branch delay slot, where
               the target takes RF exception and gets an I-cache miss
               for the exception vector or where I-cache miss occurs for
               the target address, under the above mentioned scenarios,
               the div would get wrong results.
            ...
            Workaround: For item 1), OS could make sure that the next
            page after the divide instruction is also mapped. For item
            2), the compiler could make sure that the divide instruction
            is not in the branch delay slot."

       These processors have PRId values of 0x00004220 and 0x00004300
       for the R4000 and 0x00004400, 0x00004500 and 0x00004600 for the
       R4400. */
...
static struct {
  ...
  /* True if current cycle has a multi instruction. This flag is used in
     mips_ls2_dfa_post_advance_cycle. */
  bool cycle_has_multi_p;

  /* Instructions to subscribe ls2_[f]alu{1,2}_turn_enabled units. These
     are used in mips_ls2_dfa_post_advance_cycle to initialize DFA
     state. E.g., when alu1_turn_enabled_insn is issued it makes next
     ALU1/2 instruction to go ALU1. */
  ...
  switch ( mips_tune ) {
    ...
    case PROCESSOR_74KF3_2:
      /* The 74k is not strictly quad-issue cpu, but can be seen as one
         ...
         floating-point loads and stores also require a slot in the AGEN
         pipe. */
    ...
    case PROCESSOR_SB1:
    case PROCESSOR_SB1A:
      /* This is actually 4, but we get better performance if we claim
         3. This is partly because of unwanted speculative code motion
         with the larger number, and partly because in most common cases
         we can't reach the theoretical max of 4. */
      return 3;
...
/* mips_ls2_dfa_post_advance_cycle                                      */
/*----------------------------------------------------------------------*/
/* Initialize STATE when scheduling for Loongson 2E/2F. Support
   round-robin dispatch scheme by enabling only one of ALU1/ALU2 and one
   of FALU1/FALU2 units for ALU1/2 and FALU1/2 instructions
   respectively. */
...
/*----------------------------------------------------------------------*/
/* Implement TARGET_SCHED_FIRST_CYCLE_MULTIPASS_DFA_LOOKAHEAD. This
   should be as wide as the scheduling freedom in the DFA. */
...
{
  if ( pos1 < pos2
       && INSN_PRIORITY( ready[pos1] ) + limit
                         >= INSN_PRIORITY( ready[pos2] ) ) 
  {
    rtx temp;
...
/* A TUNE_MACC_CHAINS helper function. Search ready queue READY, which
   ...
   instruction that is cumulative with mips_macc_chains_last_hilo. If
   there is one, promote it ahead of anything else that might clobber hi
   or lo. */
...
/* A note_stores callback used by vr4130_true_reg_dependence_p. DATA
   points to an rtx that is initially an instruction. Nullify the rtx if
   the instruction uses the value of register X. */
...
/*----------------------------------------------------------------------*/
/* Return true if there is true register dependence between
   vr4130_last_insn and INSN. */
...
/* A TUNE_MIPS4130 helper function. Given that INSN1 is at the head of
   ...
   true if it is worth promoting INSN2 ahead of INSN1. Look for cases in
   which INSN1 and INSN2 can probably issue in parallel, but for which
   (INSN2, INSN1) should be less sensitive to instruction alignment than
   (INSN1, INSN2). See 4130.md for more details. */
...
vr4130_swap_insns_p( rtx insn1, rtx insn2 )
{
  ...
  /* Check for the following case:
     ...
     1) there is some other instr X with an anti dependence on INSN1;
     2) X has a higher priority than INSN2; and
     3) X is an arithmetic instr (and thus has no unit restrictions).
  ...
  if ( vr4130_last_insn != 0
    ...
    /* See whether INSN1 and INSN2 use different execution units, or if
       they are both ALU-type instructions. If so, they can probably
       execute in parallel. */
    ...
    if ( class1 != class2 || class1 == VR4130_CLASS_ALU ) {
      ...
      /* Prefer to schedule INSN2 ahead of INSN1 if vr4130_last_insn is
         not an ALU-type instruction and if INSN1 uses the same
...
/* A TUNE_74K helper function. The 74K AGEN pipeline likes multiple
   ...
   together. Swap things around in the ready queue to make this
   happen. */
...
static void
mips_sched_init( FILE *file ATTRIBUTE_UNUSED, 
                 ...
                 int max_ready ATTRIBUTE_UNUSED )
{
  ...
  /* When scheduling for Loongson2, branch instructions go to ALU1,
     therefore basic block is most likely to start with round-robin
...
static int
mips_sched_reorder( FILE *file ATTRIBUTE_UNUSED, 
                    int verbose ATTRIBUTE_UNUSED,
                    rtx *ready, int *nreadyp, 
...
mips_ls2_variable_issue( rtx insn )
{
  if ( mips_ls2.alu1_turn_p ) {
    if ( cpu_unit_reservation_p( curr_state, 
                                 mips_ls2.alu1_core_unit_code ) )
      mips_ls2.alu1_turn_p = false;
  }
  else {
    if ( cpu_unit_reservation_p( curr_state, 
                                 mips_ls2.alu2_core_unit_code ) )
      mips_ls2.alu1_turn_p = true;
  }

  if ( mips_ls2.falu1_turn_p ) {
    if ( cpu_unit_reservation_p( curr_state, 
                                 mips_ls2.falu1_core_unit_code ) )
      mips_ls2.falu1_turn_p = false;
  }
  else {
    if ( cpu_unit_reservation_p( curr_state, 
...
static int
mips_variable_issue( FILE *file ATTRIBUTE_UNUSED, 
                     int verbose ATTRIBUTE_UNUSED,
                     rtx insn, int more )
{
  /* Ignore USEs and CLOBBERs; don't count them against the issue
     rate. */
  if ( USEFUL_INSN_P( insn ) ) {
    more--;
  ...
  /* Instructions of type 'multi' should all be split before the second
     scheduling pass. */
...
/* Declare an availability predicate for built-in functions that require
   non-MIPS16 mode and also require COND to be true. NAME is the main
   part of the predicate's name. */
...
/* CMP_PS_BUILTINS                                                      */
/*----------------------------------------------------------------------*/
/* Define __builtin_mips_{any,all,upper,lower}_<INSN>_<COND>_ps. The
   lower and upper forms are subject to mips_builtin_avail_<AVAIL> while
   the any and all forms are subject to mips_builtin_avail_mips3d. */
...
/* CMP_BUILTINS                                                         */
/*----------------------------------------------------------------------*/
/* Define all the built-in functions related to C.cond.fmt condition
   COND. */
...
/*----------------------------------------------------------------------*/
/* DIRECT_NO_TARGET_BUILTIN                                             */
/*----------------------------------------------------------------------*/
/* Define __builtin_mips_<INSN>, which is a
   MIPS_BUILTIN_DIRECT_NO_TARGET function mapped to instruction
   CODE_FOR_mips_<INSN>, FUNCTION_TYPE and AVAIL are as for
   MIPS_BUILTIN. */
...
/*----------------------------------------------------------------------*/
/* LOONGSON_BUILTIN_ALIAS                                               */
/*----------------------------------------------------------------------*/
/* Define a Loongson MIPS_BUILTIN_DIRECT function
   __builtin_loongson_<FN_NAME> for instruction
   CODE_FOR_loongson_<INSN>. FUNCTION_TYPE is a builtin_description
   field. */
...
/*----------------------------------------------------------------------*/
/* Define a Loongson MIPS_BUILTIN_DIRECT function
   __builtin_loongson_<INSN> for instruction CODE_FOR_loongson_<INSN>.
...
/*----------------------------------------------------------------------*/
/* Like LOONGSON_BUILTIN, but add _<SUFFIX> to the end of the function
   name. We use functions of this form when the same insn can be
   usefully applied to more than one datatype. */
...
static rtx
mips_prepare_builtin_target( enum insn_code icode, 
                             unsigned int op, rtx target )
{
  enum machine_mode mode;

  mode = insn_data[icode].operand[op].mode;
  if ( target == 0 
       || !insn_data[icode].operand[op].predicate( target, mode ) )
...
/*----------------------------------------------------------------------*/
/* Expand a MIPS_BUILTIN_DIRECT or MIPS_BUILTIN_DIRECT_NO_TARGET
   function; HAS_TARGET_P says which. EXP is the CALL_EXPR that calls
   the function and ICODE is the code of the associated .md pattern.
...
/*----------------------------------------------------------------------*/
/* Move VALUE_IF_TRUE into TARGET if CONDITION is true; move
   VALUE_IF_FALSE into TARGET otherwise. Return TARGET. */
                              ...
                              rtx value_if_true, rtx value_if_false )
{
  ...
  /* Branch to TRUE_LABEL if CONDITION is true and DONE_LABEL
     otherwise. */
                             ...
                             rtx target, tree exp )
{
  ...
  /* If the comparison sets more than one register, we define the result
     to be 0 if all registers are false and -1 if all registers are
     true. The value of the complete result is indeterminate
     otherwise. */
  switch ( builtin_type ) {
...
static rtx
mips_expand_builtin_bposge( enum mips_builtin_type builtin_type, 
                            rtx target )
{
                     ...
                     rtx value, enum machine_mode mode )
{
  ...
  /* See whether the constant is already in the pool. If so, return the
     ...
     Keep the pool sorted in increasing order of mode size so that we
     can reduce the number of alignments needed. */
  ...
  if ( pool->first == 0 )
    ...
    pool->highest_address
      = pool->insn_address - ( UNITS_PER_WORD - 2 ) + 0x8000;
...
/*----------------------------------------------------------------------*/
/* X is an expression that appears in INSN. Try to use the UD chains to
   simplify it, returning the simplified form on success and the
   original form otherwise. Replace the incoming value of $sp with
   virtual_incoming_args_rtx (which should never occur in X
   otherwise). */
...
r10k_simplify_address( rtx x, rtx insn )
{
  ...
  if ( UNARY_P( x ) ) {
    ...
    /* LO_SUMs can be offset from HIGHs, if we know they won't overflow.
       See mips_classify_address for the rationale behind the lax
       check. */
    op0 = r10k_simplify_address( XEXP( x, 0 ), insn );
...
/* r10k_safe_address_p                                                  */
/*----------------------------------------------------------------------*/
/* Return true if we can prove that an access to address X in
   instruction INSN would be safe from R10K speculation. This X is a
   general expression; it might not be a legitimate address. */
...
r10k_safe_address_p( rtx x, rtx insn )
{
  ...
  /* Check for references to the stack frame. It doesn't really matter
     how much of the frame has been allocated at INSN;
     -mr10k-cache-barrier allows us to assume that accesses to any part
     of the eventual frame is safe from speculation at any point in the
     function. */
...
/*----------------------------------------------------------------------*/
/* A note_stores callback for which DATA points to an instruction
   pointer. If *DATA is nonnull, make it null if it X contains a MEM
   that is not safe from R10K speculation. */
                               ...
                               void *data )
{
  ...
  insn_ptr = ( rtx * ) data;
  if ( *insn_ptr && for_each_rtx( &x, r10k_needs_protection_p_1,
                                  *insn_ptr ) )
    *insn_ptr = NULL_RTX;
}
...
/*----------------------------------------------------------------------*/
/* A for_each_rtx callback that iterates over the pattern of a
   CALL_INSN. Return nonzero if the call is not to a declared
   function. */
...
/* r10k_protected_bb_p                                                  */
/*----------------------------------------------------------------------*/
/* Return true if BB is only reached by blocks in PROTECTED_BBS and if
   every edge is unconditional. */
    ...
    for ( insn = BB_HEAD( bb ); insn != end; insn = NEXT_INSN( insn ) ) {
      if ( unprotected_region && INSN_P( insn ) ) {
        if ( recog_memoized( insn ) == CODE_FOR_mips_cache )
          ...
          /* See if INSN needs to be protected. If so, we must insert a
             cache barrier somewhere between PREV_INSN
             (UNPROTECTED_REGION) and INSN. It isn't clear which
             position is better performance-wise, but as a tie-breaker,
             we assume that it is better to allow delay slots to be
             back-filled where possible, and that it is better not to
             insert barriers in the middle of already-scheduled code. We
             therefore insert the barrier at the beginning of the
             region. */
...
/* A structure representing the state of the processor pipeline. Used by
   the mips_sim_* family of functions. */
struct mips_sim 
{
...
/*----------------------------------------------------------------------*/
/* A for_each_rtx callback. If *X is a register, advance simulation
...
/*----------------------------------------------------------------------*/
/* Call mips_sim_wait_regs_2 (R, DATA) for each register R mentioned in
   *X. */
...
/*----------------------------------------------------------------------*/
/* Update simulation state STATE so that it's ready to accept the
   instruction after INSN. INSN should be part of the main rtl chain,
  ...
  switch ( GET_CODE( SEQ_BEGIN( insn ) ) ) {
    ...
    case JUMP_INSN:
      /* The delay slots of branch likely instructions are only executed
         when the branch is taken. Therefore, if the caller has
         simulated the delay slot instruction, STATE does not really
         reflect the state of the pipeline for the instruction after the
         delay slot. Also, branch likely instructions tend to incur a
         penalty when not taken, so there will probably be an extra
         delay between the branch and the instruction after the delay
         slot. */
...
/* The VR4130 pipeline issues aligned pairs of instructions together,
   but it stalls the second instruction if it depends on the first. In
   order to cut down the amount of logic required, this dependence check
   is not based on a full instruction decode. Instead, any non-SPECIAL
   ...
   and its delay slot. If this situation occurs in instruction INSN, try
   to avoid it by swapping rs and rt. */
...
/* vr4130_align_insns                                                   */
/*----------------------------------------------------------------------*/
/* Implement -mvr4130-align. Go through each basic block and simulate
   the processor pipeline. If we find that a pair of instructions could
   execute in parallel, and the first of those instructions is not
   8-byte aligned, insert a nop to make it aligned. */
...
vr4130_align_insns( void )
{
  ...
  for ( insn = get_insns(); insn != 0; insn = next ) {
    ...
    /* See the comment above vr4130_avoid_branch_rt_conflict for
       details. This isn't really related to the alignment pass, but we
       do it on the fly to avoid a separate instruction walk. */
      ...
      if ( state.insns_left != state.issue_rate
        ...
        if ( subinsn == SEQ_BEGIN( insn ) && aligned_p ) {
          /* SUBINSN is the first instruction in INSN and INSN is
             aligned. We want to align the previous instruction instead,
             so insert a nop between LAST2 and LAST.

             Note that LAST could be either a single instruction or a
             branch with a delay slot. In the latter case, LAST, like
             INSN, is already aligned, but the delay slot must have some
             extra delay that stops it from issuing at the same time as
             the branch. We therefore insert a nop before the branch in
             order to align its delay slot. */
          emit_insn_after( gen_nop(), last2 );
          aligned_p = false;
        }
        else if ( subinsn != SEQ_BEGIN( insn ) && !aligned_p ) {
          /* SUBINSN is the delay slot of INSN, but INSN is currently
             unaligned. Insert a nop between LAST and INSN to align
             it. */
    ...
    if ( length > 0 ) {
      /* If the instruction is an asm statement or multi-instruction
         mips.md patern, the length is only an estimate. Insert an 8
         byte alignment after it so that the following instructions can
...
/* This structure records that the current function has a LO_SUM
   involving SYMBOL_REF or LABEL_REF BASE and that MAX_OFFSET is the
   largest offset applied to BASE by all such LO_SUMs. */
...
/* mips_record_lo_sum                                                   */
/*----------------------------------------------------------------------*/
/* A for_each_rtx callback for which DATA is a mips_lo_sum_offset hash
   table. Record every LO_SUM in *LOC. */
...
mips_orphaned_high_part_p( htab_t htab, rtx insn )
{
  ...
  if ( set ) {
    ...
    /* Check for local %gots (and %got_pages, which is redundant but
       OK). */
...
/* Subroutine of mips_reorg_process_insns. If there is a hazard between
   ...
   *DELAYED_REG and *HILO_DELAY describe the hazards that apply at this
   point. If *DELAYED_REG is non-null, INSN must wait a cycle before
   using the value of that register. *HILO_DELAY counts the number of
   instructions since the last hilo hazard (that is, the number of
   instructions since the last MFLO or MFHI).
                   ...
                   rtx *delayed_reg, rtx lo_reg )
{
  ...
  /* Do not put the whole function in .set noreorder if it contains an
     asm statement. We don't know whether there will be hazards between
     the asm statement and the gcc-generated code. */
  ...
  /* Work out how many nops are needed. Note that we only care about
     ...
     pattern. It doesn't matter that calls use the argument registers or
     that they clobber hi and lo. */
  ...
  /* Insert the nops between this instruction and the previous one. Each
     new nop takes us further from the last hilo hazard. */
  *hilo_delay += nops;
  while ( nops-- > 0 )
...
/* Go through the instruction stream and insert nops where necessary.
   Also delete any high-part relocations whose partnering low parts are
   now all dead. See if the whole function can then be put into .set
   noreorder and .set nomacro. */
...
mips_reorg_process_insns( void )
{
  ...
  /* The same is true for -mfix-vr4130 if we might generate MFLO or MFHI
     instructions. Note that we avoid using MFLO and MFHI if the VR4130
     MACC and DMACC instructions are available instead; see the
     *mfhilo_{si,di}_macc patterns. */
...
/*----------------------------------------------------------------------*/
/* Implement TARGET_ASM_OUTPUT_MI_THUNK. Generate rtl rather than asm
   text in order to avoid duplicating too much logic from elsewhere. */
                      ...
                      tree function )
{
  ...
  if ( use_sibcall_p ) {
    ...
    /* This is messy. GAS treats "la $25,foo" as part of a call
       ...
       In this context, lazy binding would actually be OK for
       TARGET_CALL_CLOBBERED_GP, but it's still wrong for
       TARGET_CALL_SAVED_GP; see mips_load_call_address. We must
       therefore load the address via a temporary register if
       mips_dangerous_for_la25_p.
    
       If we jump to the temporary register rather than $25, the
       assembler can use the move insn to fill the jump's delay slot.
    
       We can use the same technique for MIPS16 code, where $25 is not a
       valid JR register. */
    ...
    if ( TARGET_USE_PIC_FN_ADDR_REG
      ...
      mips_emit_move( gen_rtx_REG( Pmode, PIC_FUNCTION_ADDR_REGNUM ), 
                      temp1 );
...
/* The last argument passed to mips_set_mips16_mode, or negative if the
   ...
   invocation. The information calculated by mips_set_mips16_mode is
   invalid unless the two variables are the same. */
...
mips_set_mips16_mode( int mips16_p )
{
  ...
  if ( mips16_p ) {
    ...
    /* Don't move loop invariants, because it tends to increase register
       pressure. It also introduces an extra move in cases where the
       constant is the first operand in a two-operand binary
    ...
    /* Experiments suggest we get the best overall section-anchor
       results from using the range of an unextended LW or SW. Code that
       makes heavy use of byte or short accesses can do better with
       ranges of 0...31 and 0...63 respectively, but most code is
...
/*----------------------------------------------------------------------*/
/* Return true if GIVEN matches CANONICAL, where GIVEN is a
   user-supplied CPU name. We've traditionally allowed a lot of
   variation here.
...
{
  /* First see if the name matches exactly, or with a final "000" turned
     into "k". */
  if ( mips_strict_matching_cpu_name_p( canonical, given ) )
    return true;

  /* If not, try comparing based on numerical designation alone. See if
     GIVEN is an unadorned number, or 'r' followed by a number. */
  if ( TOLOWER( *given ) == 'r' )
    given++;
  if ( !ISDIGIT( *given ) )
    return false;
...
mips_parse_cpu( const char* cpu_string ) {
  ...
  /* 'default' has traditionally been a no-op. Probably not very
     useful. */
  ...
  switch ( code ) {
    ...
    case OPT_mips:
      mips_isa_option_info
        = mips_parse_cpu( ACONCAT(( "mips", arg, NULL ) ) );
...
mips_override_options( void )
{
  ...
  /* The following code determines the architecture and register size.
     Similar code was added to GAS 2.14 (see
     tc-mips.c:md_after_parse_args()). The GAS and GCC code should be
     kept in sync as much as possible. */
  ...
  /* Optimize for mips_arch, unless -mtune selects a different
     processor. */
  ...
  /* If neither -mbranch-likely nor -mno-branch-likely was given on the
     command line, set MASK_BRANCHLIKELY based on the target
     architecture and tuning flags. Annulled delay slots are a size win,
     so we only consider the processor-specific tuning for
     !optimize_size. */
  ...
  if ( TARGET_ABICALLS_PIC2 )
    /* We need to set flag_pic for executables as well as DSOs because
       we may reference symbols that are not defined in the final
       executable. (MIPS does not use things like copy relocs, for
       example.)

       There is a body of code that uses __PIC__ to distinguish between
       -mabicalls and -mno-abicalls code. The non-__PIC__ variant is
       usually appropriate for TARGET_ABICALLS_PIC0, as long as any
       indirect jumps use $25. */
    flag_pic = 1;

  /* -mvr4130-align is a "speed over size" optimization: it usually
     produces faster code, but at the expense of more nops. Enable it at
     -O3 and above. */
  ...
  /* Make sure that when TARGET_PAIRED_SINGLE_FLOAT is true,
     TARGET_FLOAT64 and TARGET_HARD_FLOAT_ABI are both true. */
  if ( TARGET_PAIRED_SINGLE_FLOAT 
       && !( TARGET_FLOAT64 && TARGET_HARD_FLOAT_ABI ) )
    error( "%qs must be used with %qs",
           TARGET_MIPS3D ? "-mips3d" : "-mpaired-single",
           TARGET_HARD_FLOAT_ABI ? "-mfp64" : "-mhard-float" );

  /* Make sure that the ISA supports TARGET_PAIRED_SINGLE_FLOAT when it
     is enabled. */
  ...
  /* .eh_frame addresses should be the same width as a C pointer. Most
     MIPS ABIs support only one pointer size, so the assembler will
     usually know exactly how big an .eh_frame address is.
     ...
     this is still the default mode. However, we also support an
     n32-like ILP32 mode, which is selected by -mlong32. The problem is
     that the assembler has traditionally not had an -mlong option, so
     it has traditionally not known whether we're using the ILP32 or
     LP64 form.
     ...
     default behavior, it seems unwise to rely on future versions of gas
     behaving the same way. We therefore avoid using .cfi directives for
     -mlong32 as well. */
  ...
  /* Default to working around R4000 errata only if the processor was
     selected explicitly. */
  if (( target_flags_explicit & MASK_FIX_R4000 ) == 0
      && mips_matching_cpu_name_p( mips_arch_info->name, "r4000" ) )
    target_flags |= MASK_FIX_R4000;

  /* Default to working around R4400 errata only if the processor was
     selected explicitly. */
  if (( target_flags_explicit & MASK_FIX_R4400 ) == 0
      && mips_matching_cpu_name_p( mips_arch_info->name, "r4400" ) )
    target_flags |= MASK_FIX_R4400;

  /* Default to working around R10000 errata only if the processor was
     selected explicitly. */
...
mips_conditional_register_usage( void )
{
  ...
  /* In MIPS16 mode, we permit the $t temporary registers to be used for
     ...
     call-saved, and saving them via a MIPS16 register would probably
     waste more time than just reloading the value. */
  if ( TARGET_MIPS16 ) {
  ...
  if ( mips_abi == ABI_N32 ) {
    int regno;
    for ( regno = FP_REG_FIRST + 21;
          regno <= FP_REG_FIRST + 31; regno += 2 )
      call_really_used_regs[regno] = call_used_regs[regno] = 1;
  }
...
mips_expand_vector_init( rtx target, rtx vals )
{
  ...
  for ( i = 0; i < n_elts; i++ )
    emit_move_insn( adjust_address_nv( mem, inner, 
                                       i * GET_MODE_SIZE(inner) ),
                    XVECEXP( vals, 0, i ) );
